        ...
        <div class="container-xl">
            <div class="card">
                <div class="card-header justify-content-between">
                    <h2 class="mb-0">{{ __('Danh sách học viên') }}</h2>
                    @if (auth()->user()->isSuperAdmin)
                        <x-form :action="route('admin.student.import')" type="post" enctype="multipart/form-data" :validate="true">
...
@push('custom-js')
    <script>
        ...
        document.getElementById('export-button').addEventListener('click', function() {
            window.location.href = "{{ route('admin.student.export') }}";
        });
    </script>
    {{ $dataTable->scripts() }}
