<script>
    window.AppConfig = {
        apiEndpoints: {
            ...
            teacherAvailableTimes: "{{ route('admin.booking.apiTeacherAvailableTimes', ['id' => ':teacherId']) }}",
        }
    };
    ...
    function removeFilter(value, type) {
        ...
        if (shouldApplyFilters) {
            ...
            if (type === 'teacher' || type === 'lesson' || type === 'date') {
                console.log(`Removed ${type} filter:`, value);

                // ✅ Nếu có callback riêng cho teacher/lesson filters
     ...
     */
    async function fetchTeacherAvailableTimes(teacherId, date) {
        const queryParams = `date=${date}`;
        const apiUrl = window.AppConfig.apiEndpoints.teacherAvailableTimes.replace(':teacherId', teacherId);
        ...
        try {
            ...
            if (!response.ok) {
                ...
                try {
                    ...
                    if (response.status === 404 && !errorData.success) {
                        return []; // Trả về mảng rỗng nếu không có khung giờ
                    }
            ...
            // Nếu API trả về dạng paginate {data: [...], current_page, ...}
            const timeSlotsData = Array.isArray(data.data) ? data.data : (Array.isArray(data.data?.data) ? data.data
                .data : []);

            // Trả về chỉ mảng slot với start_time
            ...
            return []; // Trả về mảng rỗng nếu có lỗi mạng/server
        }
    }
...
;
    ...
    document.getElementById('teacherConfirmSelectionButton').addEventListener('click', async () => {
        ...
        const firstSelectedTeacher = selectedTeachers[0]; // Đổi tên biến cho rõ ràng
        const slots = await fetchTeacherAvailableTimes(firstSelectedTeacher.id, selectedDate);
    ...
    `;
        ...
        $('#filterOffcanvas').off('show.bs.offcanvas').on('show.bs.offcanvas', async function() {
            ...
            const fetchAndDisplayTimeSlots = async (teacherId) => {
                ...
                if (isLoadingTimeSlots) {
                    console.log('Time slots đang được load, bỏ qua request này');
                    return;
                }
                ...
                try {
                    ...
                    console.log(
                        `🔥 Gọi API lessons: ${apiUrl}?${queryParams}`); // ✅ Debug log
            ...
            // ✅ Gọi 1 lần khi mở offcanvas
            await fetchAndDisplayTeachers(currentFilters.date);

            // ✅ KHÔNG re-apply ở đây nữa vì đã xử lý trong fetchAndDisplayTimeSlots
            // Nếu cần re-apply, làm sau khi fetchAndDisplayTimeSlots hoàn thành
        });
        ...
        document.getElementById('teacherConfirmSelectionButton').addEventListener('click', async () => {
            ...
            const slots = await fetchTeacherAvailableTimes(first.id, selectedDate);
