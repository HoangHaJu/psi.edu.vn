...
@section('title', 'Game 1 - Unit 1: Who’s in your family?')
...
@push('styles')
    ...
    <style>
        /* Popup style — có thể tách ra file riêng nếu dùng chung cho nhiều unit */
        .popup {
            position: fixed;
            inset: 0;
...
@section('content')
    ...
    <main class="game-content">
        <div class="top-bar">
            ...
            {{-- Navigation --}}
            <a href="#" class="btn nav disabled"><span class="arrow">&laquo;</span></a>
            <a href="{{ route('admin.game_psi.unit.show', ['game' => 1, 'unit' => 2]) }}" class="btn nav"><span
        ...
        <div class="lesson-box">
            <div class="lesson-header">
                <img src="{{ asset('admin/assets/images/game_psi/image_general/Frame69.png') }}" class="touch-icon"
                    alt="touch icon">
                <h2>Unit 1: Who's in your family?</h2>
            </div>
            ...
            <div class="audio-buttons">
                @php
                    $audioData = [
                        ['answer' => 'brother', 'file' => 'brother.mp3'],
                        ['answer' => 'dad', 'file' => 'dad.mp3'],
                        ['answer' => 'sister', 'file' => 'sister.mp3'],
                        ['answer' => 'mom', 'file' => 'mom.mp3'],
                    ];
                @endphp

                @foreach ($audioData as $audio)
                    <button class="audio" data-answer="{{ $audio['answer'] }}"
                        data-sound="{{ asset("admin/assets/images/game_psi/game1/unit1/sound/{$audio['file']}") }}">
            ...
            <div class="characters">
                @foreach (['brother', 'dad', 'mom', 'sister'] as $character)
                    <div class="character" data-name="{{ $character }}">
                        <img src="{{ asset("admin/assets/images/game_psi/game1/unit1/img/{$character}.jpg") }}"
                            alt="{{ $character }}">
                    </div>
                @endforeach
            </div>
...
@push('scripts')
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            const answers = ["brother", "dad", "sister", "mom"];
