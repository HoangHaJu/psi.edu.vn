...
@section('title', 'Game 1 - Unit 2: What\'s this?')
...
@section('content')
    ...
    <main class="game-content">
        <div class="top-bar">
            ...
            {{-- Navigation --}}
            <a href="{{ route('admin.game_psi.unit.show', ['game' => 1, 'unit' => 1]) }}" class="btn nav"><span
                    class="arrow">&laquo;</span></a>
            <a href="{{ route('admin.game_psi.unit.show', ['game' => 1, 'unit' => 3]) }}" class="btn nav"><span
        ...
        <div class="lesson-box">
            ...
            <div class="audio-buttons">
                @php
                    $audioData = [
                        ['answer' => 'book', 'file' => 'book.mp3'],
                        ['answer' => 'pencil', 'file' => 'pencil.mp3'],
                        ['answer' => 'bag', 'file' => 'bag.mp3'],
                        ['answer' => 'chair', 'file' => 'chair.mp3'],
                    ];
                @endphp

                @foreach ($audioData as $audio)
                    <button class="audio" data-answer="{{ $audio['answer'] }}"
                        data-sound="{{ asset("admin/assets/images/game_psi/game1/unit2/sound/{$audio['file']}") }}">
            ...
            <div class="characters">
                @foreach (['book', 'pencil', 'bag', 'chair'] as $character)
                    <div class="character" data-name="{{ $character }}">
                        <img src="{{ asset("admin/assets/images/game_psi/game1/unit2/img/{$character}.jpg") }}"
                            alt="{{ $character }}">
                    </div>
                @endforeach
            </div>
...
@push('scripts')
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            const answers = ["book", "pencil", "bag", "chair"];
