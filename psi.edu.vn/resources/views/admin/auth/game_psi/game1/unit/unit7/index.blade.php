...
@section('title', 'Game 1 - Unit 7: What pets do you like?')
...
@section('content')
    ...
    <main class="game-content">
        <div class="top-bar">
            ...
            {{-- Navigation --}}
            <a href="{{ route('admin.game_psi.unit.show', ['game' => 1, 'unit' => 6]) }}" class="btn nav"><span
                    class="arrow">&laquo;</span></a>
            <a href="{{ route('admin.game_psi.unit.show', ['game' => 1, 'unit' => 8]) }}" class="btn nav"><span
        ...
        <div class="lesson-box">
            <div class="lesson-header">
                <img src="{{ asset('admin/assets/images/game_psi/image_general/Frame69.png') }}" class="touch-icon"
                    alt="touch icon">
                <h2>Unit 7: What pets do you like?</h2>
            </div>
            ...
            <div class="audio-buttons">
                @php
                    // CẬP NHẬT: Thay thế dog/fish bằng rabbit/turtle
                    $audioData = [
                        ['answer' => 'bird', 'file' => 'bird.mp3'], // Giả định bird.mp3
                        ['answer' => 'cat', 'file' => 'cat.mp3'], // Giả định cat.mp3
                        ['answer' => 'rabbit', 'file' => 'rabbit.mp3'], // Giả định rabbit.mp3
                        ['answer' => 'turtle', 'file' => 'turtle.mp3'], // Giả định turtle.mp3
                    ];
                @endphp

                @foreach ($audioData as $audio)
                    <button class="audio" data-answer="{{ $audio['answer'] }}"
                        data-sound="{{ asset("admin/assets/images/game_psi/game1/unit7/sound/{$audio['file']}") }}">
            ...
            <div class="characters">
                {{-- CẬP NHẬT: Chỉ sử dụng tên tệp từ ảnh mới: bird, cat, rabbit, turtle --}}
                @foreach (['bird', 'cat', 'rabbit', 'turtle'] as $character)
                    <div class="character" data-name="{{ $character }}">
                        <img src="{{ asset("admin/assets/images/game_psi/game1/unit7/img/{$character}.jpg") }}"
                            alt="{{ $character }}">
                    </div>
                @endforeach
            </div>
...
@push('scripts')
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            // CẬP NHẬT: Thay thế danh sách đáp án
            const answers = ["bird", "cat", "rabbit", "turtle"];
