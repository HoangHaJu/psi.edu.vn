...
@php
    $game_number = 3;
    $unit_number = 6;
    $unit_title = 'Unit 6: Which animals live in the wild?';

    // Dữ liệu cho game Match: [answer] là tên từ, [file] là tên file audio.
    // Lưu ý: data-answer trong HTML gốc là "crocodiles" nhưng file âm thanh là "crocodile.mp3".
    // Chúng ta sẽ giữ data-answer là 'crocodiles' (số nhiều) để khớp với data-name của hình ảnh.
    $items = [
        ['answer' => 'elephant', 'file' => 'elephant.mp3'],
        ['answer' => 'giraffe', 'file' => 'giraffe.mp3'],
        ['answer' => 'lion', 'file' => 'lion.mp3'],
        ['answer' => 'crocodiles', 'file' => 'crocodile.mp3'], // Dùng file 'crocodile.mp3'
        ['answer' => 'parrot', 'file' => 'parrot.mp3'],
        ['answer' => 'monkey', 'file' => 'monkey.mp3'],
    ];
...
@section('content')
    ...
    <main class="game-content">
        <div class="top-bar">
            ...
            {{-- Navigation: Unit tiếp theo (Unit 7) --}}
            <a href="{{ route('admin.game_psi.unit.show', ['game' => $game_number, 'unit' => $unit_number + 1]) }}">
        ...
        <div class="lesson-box">
            ...
            <div class="audio-buttons">
                @foreach ($items as $index => $item)
                    {{-- Chỉ có icon loa --}}
                    <button class="audio" data-answer="{{ $item['answer'] }}" {{-- Đường dẫn audio: /admin/assets/images/game3/unit6/sound/{file} --}}
            ...
            <div class="characters">
                <div class="character" data-name="crocodiles">
                    <img src="{{ asset("admin/assets/images/game_psi/game{$game_number}/unit{$unit_number}/img/crocodiles.jpg") }}"
                        alt="crocodiles" />
                </div>
                <div class="character" data-name="elephant">
                    <img src="{{ asset("admin/assets/images/game_psi/game{$game_number}/unit{$unit_number}/img/elephant.jpg") }}"
                        alt="elephant" />
                </div>
                <div class="character" data-name="giraffe">
                    <img src="{{ asset("admin/assets/images/game_psi/game{$game_number}/unit{$unit_number}/img/giraffe.jpg") }}"
                        alt="giraffe" />
                </div>
                <div class="character" data-name="lion">
                    <img src="{{ asset("admin/assets/images/game_psi/game{$game_number}/unit{$unit_number}/img/lion.jpg") }}"
                        alt="lion" />
                </div>
                <div class="character" data-name="monkey">
                    <img src="{{ asset("admin/assets/images/game_psi/game{$game_number}/unit{$unit_number}/img/monkey.jpg") }}"
                        alt="monkey" />
                </div>
                <div class="character" data-name="parrot">
                    <img src="{{ asset("admin/assets/images/game_psi/game{$game_number}/unit{$unit_number}/img/parrot.jpg") }}"
                        alt="parrot" />
                </div>
            </div>
