...
@php
    $game_number = 3;
    $unit_number = 5;
    $unit_title = 'Unit 5: How do you have fun?';

    // Dữ liệu cho game Match: [answer] là tên từ, [file] là tên file audio.
    // Lưu ý: Các giá trị có khoảng trắng phải được xử lý cẩn thận trong JS.
    $items = [
        ['answer' => 'dance', 'file' => 'dance.mp3'],
        ['answer' => 'ride a scooter', 'file' => 'ride_a_scooter.mp3'], // Có khoảng trắng trong answer
        ['answer' => 'run', 'file' => 'run.mp3'],
        ['answer' => 'jump rope', 'file' => 'jump_rope.mp3'], // Có khoảng trắng trong answer
        ['answer' => 'ride a bike', 'file' => 'ride_a_bike.mp3'], // Có khoảng trắng trong answer
        ['answer' => 'roller skate', 'file' => 'roller_skate.mp3'], // Có khoảng trắng trong answer
    ];
...
@push('styles')
    ...
    <style>
        ...
        .audio strong.p {
            font-size: 1.2em;
            font-weight: bold;
            /* Thay thế dấu gạch dưới bằng khoảng trắng để hiển thị thân thiện hơn */
...
@section('content')
    ...
    <main class="game-content">
        <div class="top-bar">
            ...
            {{-- Navigation: Unit tiếp theo (Unit 6) --}}
            <a href="{{ route('admin.game_psi.unit.show', ['game' => $game_number, 'unit' => $unit_number + 1]) }}">
        ...
        <div class="lesson-box">
            ...
            <div class="audio-buttons">
                @foreach ($items as $item)
                    <button class="audio" data-answer="{{ $item['answer'] }}" {{-- Đường dẫn audio: /admin/assets/images/game3/unit5/sound/{file} --}}
                        data-sound="{{ asset("admin/assets/images/game_psi/game{$game_number}/unit{$unit_number}/sound/{$item['file']}") }}">
                        <strong class="p">{{ $item['answer'] }}</strong>
                    </button>
                @endforeach
            </div>

            {{-- Hình nhân vật (Giữ nguyên thứ tự trong HTML gốc, đảm bảo data-name khớp với answer) --}}
            <div class="characters">
                <div class="character" data-name="ride a bike">
                    <img src="{{ asset("admin/assets/images/game_psi/game{$game_number}/unit{$unit_number}/img/ride_a_bike.jpg") }}"
                        alt="ride a bike" />
                </div>
                <div class="character" data-name="dance">
                    <img src="{{ asset("admin/assets/images/game_psi/game{$game_number}/unit{$unit_number}/img/dance.jpg") }}"
                        alt="dance" />
                </div>
                <div class="character" data-name="ride a scooter">
                    <img src="{{ asset("admin/assets/images/game_psi/game{$game_number}/unit{$unit_number}/img/ride_a_scooter.jpg") }}"
                        alt="ride a scooter" />
                </div>
                <div class="character" data-name="jump rope">
                    <img src="{{ asset("admin/assets/images/game_psi/game{$game_number}/unit{$unit_number}/img/jump_rope.jpg") }}"
                        alt="jump rope" />
                </div>
                <div class="character" data-name="run">
                    <img src="{{ asset("admin/assets/images/game_psi/game{$game_number}/unit{$unit_number}/img/run2.jpg") }}"
                        alt="run" />
                </div>
                <div class="character" data-name="roller skate">
                    <img src="{{ asset("admin/assets/images/game_psi/game{$game_number}/unit{$unit_number}/img/roller_skate.jpg") }}"
                        alt="roller skate" />
                </div>
            </div>
...
@push('scripts')
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            // Lấy danh sách đáp án (Thứ tự phải khớp với thứ tự các nút audio)
            ...
            characterImages.forEach(character => {
                character.addEventListener('click', () => {
                    ...
                    // So sánh: chuyển tất cả về chữ thường và xóa khoảng trắng thừa ở hai đầu nếu có
                    const selected = character.dataset.name.toLowerCase().trim();
                    const correct = answers[currentAudioIndex].toLowerCase().trim();
