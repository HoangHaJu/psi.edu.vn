...
class FileService
{
    ...
    public function uploadAudio(UploadedFile $audioFile)
    {
        // Kiểm tra loại file là audio (mp3, wav)
        if (!in_array($audioFile->getClientOriginalExtension(), ['mp3', 'wav'])) {
            throw new \Exception('Lỗi định dạng. Chỉ có file MP3 và WAV mới được cho phép.');
        }

        $this->setFolder('audio');

        $path = $audioFile->storeAs($this->folder, $audioFile->hashName(), $this->disk);
        $this->instance = $this->folderPrefix . $path;
